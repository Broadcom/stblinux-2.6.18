...
#ifdef	CONFIG_SATA_FORCE_SPINUP	// jipeng - here we need to insert a command to spin up some Seagate HDD
/**
 *	sata_dev_set_xfermode_spinup - Issue SET FEATURES - XFER MODE command
...
static unsigned int sata_dev_set_xfermode_spinup(struct ata_device *dev)
{
	struct ata_taskfile tf;
	unsigned int err_mask;
...
#ifdef	CONFIG_SATA_FORCE_SPINUP	// jipeng - here we need to insert a command to spin up some Seagate HDD
	err_mask = sata_dev_set_xfermode_spinup(dev);
	if (err_mask) {
