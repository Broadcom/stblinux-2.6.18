/*
 * arch/mips/brcmstb/brcm97443A0/board.c
 *
 ...
 * when         who    what
 * ----         ---    ----
 * 06-04-2008   THT    Modified from 7325A0
 */
...
static bcm_discontigMem_t bcm7440b0_discontig_memmap;
...
#define BCM7440B0_UPPER_RAM_BASE 0x20000000
...
static void bcm7440b0_memmap_init(void);
...
board_init_once(void)
{
	...
	*/
	...
	memset(&bcm7440b0_discontig_memmap, 0, sizeof(bcm_discontigMem_t));
	bcm_pdiscontig_memmap = &bcm7440b0_discontig_memmap;
	...
	if (!configured_from_cfe) {
		...
		/*
		** Note that the 97440B0 has only 2 memory controllers.
		*/
		//bcm_pdiscontig_memmap->numNodes = 2;
		...
		switch (ddr0 & 3) {
		case 0:
			/* 16Mx16 parts, 32MB per part */
			bcm_pdiscontig_memmap->memSize[0] = (32 * mem_chip_cnt[0]) << 20; 
			break;
		case 1:
			/* 32Mx16 parts, 64MB per part */
			bcm_pdiscontig_memmap->memSize[0] = (64 * mem_chip_cnt[0]) << 20; 
			break;
		case 2:
			/* 64Mx16 parts, 128MB per part */
		case 3:
			/* 128Mx8 parts, 128MB per part */
			bcm_pdiscontig_memmap->memSize[0] = (128 * mem_chip_cnt[0]) << 20; 
			break;
		}
		bcm_pdiscontig_memmap->physAddr[0] = 0UL;
		...
		case 0:
			/* 16Mx16 parts, 32MB per part */
			bcm_pdiscontig_memmap->memSize[1] = (32 * mem_chip_cnt[1]) << 20; 
			break;
		case 1:
			/* 32Mx16 parts, 64MB per part */
			bcm_pdiscontig_memmap->memSize[1] = (64 * mem_chip_cnt[1]) << 20; 
			break;
		case 2:
			/* 64Mx16 parts, 128MB per part */
		case 3:
			/* 128Mx8 parts, 128MB per part */
			bcm_pdiscontig_memmap->memSize[1] = (128 * mem_chip_cnt[1]) << 20; 
			break;
		}
		/*
		 * THT: Ideally, we should define UPPER_RAM_BASE in terms of bcm_pdiscontig_memmap->physAddr[1],
		 * so that each board can define its own specific physical memory map,
		 * and not the other way around, but I don't want to break existing codes
		 */
		bcm_pdiscontig_memmap->physAddr[1] = BCM7440B0_UPPER_RAM_BASE;
 ...
 */
static bcm_memmap_t bcm7440b0_memmap = {
	.tlb_mask =		PM_64M,
	.pci_vAddr =		PCI_MEM_WIN_BASE,		/* 0xd000_0000 */
	.pci_winSize =		PCI_MEM_WIN_SIZE,		/* 128MB for 7440 */
	.io_vAddr =		PCI_IO_WIN_BASE,			/* 0xf8000000 */
	.io_winSize =		PCI_IO_WIN_SIZE,			/* 0x02000000 */
	.nMemBanks =		1,
	.mem_vAddr =		{UPPER_RAM_VBASE,0},
	.tlb_memmap =	bcm7440b0_tlb_memmap,
	.tlb_pci_bridge =	bcm7440b0_pci_bridge,
	.tlb_sata_bridge =	bcm7440b0_sata_bridge
};
...
static __init void 
bcm7440b0_default_boot_mem(void)
{
	...
	if (bcm_pdiscontig_memmap->memSize[0] < (256 << 20)) {
		/*
		** 7440B0 Default Mem Map (384M memory)
		**
		...
		*/
		...
		/*
		** 7440B0 Default Mem Map (512M memory)
		**
		**  User-defined physical RAM map:
...
//brcm_print_memory_map();
		...
		/*
		** 7440B0 Default Mem Map (786M memory)
		**
		**  User-defined physical RAM map:
		**  node [00000000, 06200000: RAM]
		**  node [06200000, 01e00000: RSVD]
...
static void 
bcm7440b0_memmap_init(void)
{
	/* Replace standard maps with bcm7440b0 specific maps */
	bcm_pmemmap = &bcm7440b0_memmap;

	// THT for now: 
	brcm_default_boot_mem = bcm7440b0_default_boot_mem;
}
