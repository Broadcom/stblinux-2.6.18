...
static void brcm_uart_enable(unsigned int irq)
{
	...
	if (irq == BCM_LINUX_UARTA_IRQ)
		...
		UPG_INTC->irqen_l |= UPG_UC_IRQ;
	else if (irq == BCM_LINUX_UARTD_IRQ)
		UPG_INTC->irqen_h |= UPG_UD_IRQ;
	else
		printk("Warning : trying to enable unknown UART IRQ:%d\n", irq);
	restore_flags(flags);
}
...
static void brcm_uart_disable(unsigned int irq)
{
	...
	if (irq == BCM_LINUX_UARTA_IRQ)
		...
		printk("Warning : trying to disable unknown UART IRQ:%d\n", irq);
	restore_flags(flags);
}
...
#if 0
	/*
	 * When UART interrupt is requested, we also register a dummy handler for UART
	 * to prevent the UPG interrupt from being disabled when the driver is rmmod'ed
	 */
	static int once = 0;

	if (0 == once && BCM_LINUX_UPG_IRQ == 0) {
		once = 1;
printk("***** request_irq(%d, upg_irq_handler)\n", BCM_LINUX_UPG_IRQ);
		request_irq(BCM_LINUX_UPG_IRQ, upg_irq_handler, SA_SHIRQ, "UART", upg_irq_handler);
	}
#endif
	...
	for (irq = 1; irq <= 32; ++irq)
	{
		...
		if ((0x1 << shift) & pendingIrqs)
		{
			if (irq == BCM_LINUX_UPG_IRQ) {
				...
				else if(UPG_INTC->irqstat_l & UPG_INTC->irqen_l & UPG_UB_IRQ) {
					do_IRQ(BCM_LINUX_UARTB_IRQ, regs);
 				}
				else if(UPG_INTC->irqstat_l & UPG_INTC->irqen_l & UPG_UC_IRQ) {
					do_IRQ(BCM_LINUX_UARTC_IRQ, regs);
 				}
				else if(UPG_INTC->irqstat_h & UPG_INTC->irqen_h & UPG_UD_IRQ) {
