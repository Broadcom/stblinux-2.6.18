...
/*
 * UARTC is the default UART..  There will be no direct UART interrupt.
 */
...
static void brcm_uart_enable(unsigned int irq)
{
	...
	{
#ifdef CONFIG_MIPS_BCM3560_IARTC
		CPUINT1C->IntrW0MaskClear = BCHP_HIF_CPU_INTR1_INTR_W0_STATUS_UPG_CPU_INTR_MASK;
		*((volatile unsigned long*)BCM_UPG_IRQ0_IRQEN) |= BCHP_IRQ0_IRQEN_uc_MASK;
	...
	for (irq=1; irq<=32; irq++)
	{
		...
		if ((0x1 << shift) & pendingIrqs)
		{
			...
			if ((shift == BCHP_HIF_CPU_INTR1_INTR_W0_STATUS_UPG_CPU_INTR_SHIFT)
					&& (*((volatile unsigned long*)BCM_UPG_IRQ0_IRQSTAT) & BCHP_IRQ0_IRQSTAT_uc_MASK) 
					&& (*((volatile unsigned long*)BCM_UPG_IRQ0_IRQEN) & BCHP_IRQ0_IRQEN_uc_MASK))
