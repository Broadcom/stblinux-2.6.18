...
static bfd_boolean gldelf32ltsmip_place_orphan (asection *s);
static void gldelf32ltsmip_finish (void);


static void
gldelf32ltsmip_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static bfd_boolean
gldelf32ltsmip_load_symbols (lang_input_statement_type *entry)
{
  int class = 0;
...
static void
gldelf32ltsmip_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32ltsmip_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32ltsmip_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needed != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32ltsmip_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32ltsmip_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32ltsmip_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32ltsmip_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32ltsmip_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32ltsmip_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32ltsmip_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32ltsmip_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32ltsmip_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32ltsmip_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf32ltsmip_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32ltsmip_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32ltsmip_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32ltsmip_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32ltsmip_find_exp_assignment (exp->binary.lhs);
      gldelf32ltsmip_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32ltsmip_find_exp_assignment (exp->trinary.cond);
      gldelf32ltsmip_find_exp_assignment (exp->trinary.lhs);
      gldelf32ltsmip_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32ltsmip_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32ltsmip_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32ltsmip_find_exp_assignment (s->assignment_statement.exp);
}
...
gldelf32ltsmip_before_allocation (void)
{
  ...
  lang_for_each_statement (gldelf32ltsmip_find_statement_assignment);
...
static bfd_boolean
gldelf32ltsmip_open_dynamic_archive
...
static bfd_boolean
gldelf32ltsmip_place_orphan (asection *s)
{
...
static void
gldelf32ltsmip_finish (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (output_bfd, &link_info);

  gldelf32ltsmip_map_segments (need_layout);
  finish_default ();
}
...
gldelf32ltsmip_get_script (int *isfile)
{
  *isfile = 1;
  ...
  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/elf32ltsmip.xu";
  else if (link_info.relocatable)
    return "ldscripts/elf32ltsmip.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf32ltsmip.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf32ltsmip.xn";
  else if (link_info.pie && link_info.combreloc
	   && link_info.relro && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf32ltsmip.xdw";
  else if (link_info.pie && link_info.combreloc)
    return "ldscripts/elf32ltsmip.xdc";
  else if (link_info.pie)
    return "ldscripts/elf32ltsmip.xd";
  else if (link_info.shared && link_info.combreloc
	   && link_info.relro && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf32ltsmip.xsw";
  else if (link_info.shared && link_info.combreloc)
    return "ldscripts/elf32ltsmip.xsc";
  else if (link_info.shared)
    return "ldscripts/elf32ltsmip.xs";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf32ltsmip.xw";
  else if (link_info.combreloc)
    return "ldscripts/elf32ltsmip.xc";
  else
    return "ldscripts/elf32ltsmip.x";
}
...
struct ld_emulation_xfer_struct ld_elf32ltsmip_emulation =
{
  ...
  gldelf32ltsmip_finish,
  NULL,
  gldelf32ltsmip_open_dynamic_archive,
  gldelf32ltsmip_place_orphan,
  NULL,
  NULL,
  gldelf32ltsmip_add_options,
