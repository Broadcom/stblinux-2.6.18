...
/* 64 bit ELF emulation code for elf64ltsmip
...
static void gldelf64ltsmip_before_parse (void);
static void gldelf64ltsmip_after_open (void);
static void gldelf64ltsmip_before_allocation (void);
static bfd_boolean gldelf64ltsmip_place_orphan (asection *s);
static void gldelf64ltsmip_finish (void);


static void
gldelf64ltsmip_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf64ltsmip_before_parse (void)
{
...
static bfd_boolean
gldelf64ltsmip_load_symbols (lang_input_statement_type *entry)
{
  int class = 0;
...
static bfd_boolean
gldelf64ltsmip_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
...
static bfd_boolean
gldelf64ltsmip_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf64ltsmip_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf64ltsmip_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf64ltsmip_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf64ltsmip_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf64ltsmip_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf64ltsmip_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf64ltsmip_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf64ltsmip_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf64ltsmip_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf64ltsmip_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf64ltsmip_find_exp_assignment (s->assignment_statement.exp);
}
...
static void
gldelf64ltsmip_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  ...
  lang_for_each_statement (gldelf64ltsmip_find_statement_assignment);
...
static bfd_boolean
gldelf64ltsmip_place_orphan (asection *s)
{
...
static void
gldelf64ltsmip_finish (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (output_bfd, &link_info);

  gldelf64ltsmip_map_segments (need_layout);
  finish_default ();
}

static char *
gldelf64ltsmip_get_script (int *isfile)
{
  *isfile = 1;

  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/elf64ltsmip.xu";
  else if (link_info.relocatable)
    return "ldscripts/elf64ltsmip.xr";
  else if (!config.text_read_only)
    return "ldscripts/elf64ltsmip.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/elf64ltsmip.xn";
  else if (link_info.pie && link_info.combreloc
	   && link_info.relro && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf64ltsmip.xdw";
  else if (link_info.pie && link_info.combreloc)
    return "ldscripts/elf64ltsmip.xdc";
  else if (link_info.pie)
    return "ldscripts/elf64ltsmip.xd";
  else if (link_info.shared && link_info.combreloc
	   && link_info.relro && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf64ltsmip.xsw";
  else if (link_info.shared && link_info.combreloc)
    return "ldscripts/elf64ltsmip.xsc";
  else if (link_info.shared)
    return "ldscripts/elf64ltsmip.xs";
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DT_BIND_NOW))
    return "ldscripts/elf64ltsmip.xw";
  else if (link_info.combreloc)
    return "ldscripts/elf64ltsmip.xc";
  else
    return "ldscripts/elf64ltsmip.x";
}
...
static bfd_boolean
gldelf64ltsmip_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldelf64ltsmip_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf64ltsmip_emulation =
{
  gldelf64ltsmip_before_parse,
  syslib_default,
  hll_default,
  mips_after_parse,
  gldelf64ltsmip_after_open,
  after_allocation_default,
  set_output_arch_default,
  ldemul_default_target,
  gldelf64ltsmip_before_allocation,
  gldelf64ltsmip_get_script,
  "elf64ltsmip",
  "elf64-tradlittlemips",
  gldelf64ltsmip_finish,
  NULL,
  gldelf64ltsmip_open_dynamic_archive,
  gldelf64ltsmip_place_orphan,
  NULL,
  NULL,
  gldelf64ltsmip_add_options,
  gldelf64ltsmip_handle_option,
  NULL,
  gldelf64ltsmip_list_options,
  gldelf64ltsmip_load_symbols,
  NULL,
  NULL
};
