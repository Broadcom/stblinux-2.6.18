...
mips_sde_frame_cache (struct frame_info *next_frame, void **this_cache)
{
  ...
  int i;
  const int sizeof_reg_t = mips_abi_regsize (current_gdbarch);
  ...
  */
  ...
  trad_frame_set_reg_addr (this_trad_cache, MIPS_PS_REGNUM + NUM_REGS,
  ...
  for (i = 0; i < MIPS_NUMREGS; i++) {
    trad_frame_set_reg_addr (this_trad_cache, i + MIPS_ZERO_REGNUM + NUM_REGS,
...
static void
mips_sde_frame_prev_register (struct frame_info *next_frame, void **this_cache,
