...
/* Function: section_is_real_overlay (SECTION)
   Returns true if SECTION is *really* and overlay, not a fake
   shadow mapping as set up by MIPS to handle cached/uncached
   mapping. */

int
section_is_real_overlay (section)
     asection *section;
{
  return section_is_overlay (section) && section_is_mapped (section) <= 1;
}
