...
value_allocate_space_in_inferior (int len)
{
  ...
  /* The above lookup will return a value structure of type TYPE_CODE_FUNC,
     but with a return type of TYPE_CODE_VOID when linking against malloc 
     without debugging symbols.  If so, change its return type to a pointer
     type.  There's a similar fix in mdebugread.c  */
  if (TYPE_CODE (TYPE_TARGET_TYPE (value_type (val))) == TYPE_CODE_VOID)
      TYPE_TARGET_TYPE (value_type (val)) = 
	make_pointer_type (TYPE_TARGET_TYPE (value_type (val)), NULL);
