...
static int gdb_os_read (host_callback *, int fd, char *, int);
...
/* The simulator stdin_read prompt string. */
static char *gdbsim_prompt;
static void
show_gdbsim_prompt (struct ui_file *file, int from_tty,
		    struct cmd_list_element *c, const char *value)
{
  fprintf_filtered (file, _("Simulator input prompt is \"%s\".\n"), value);
}
...
static SIGJMP_BUF gdbsim_readline_env;
static RETSIGTYPE (*gdbsim_readline_prev_sigint) ();

static void
input_hook_signal_handler ()
{
#if 0
  printf_unfiltered ("input_hook_signal_handler\n");
  rl_cleanup_after_signal ();
#endif
  (*gdbsim_readline_prev_sigint) (SIGINT);
  SIGLONGJMP (gdbsim_readline_env, 1);
}


static int 
gdb_os_read_stdin (p, buf, len)
     host_callback *p;
     char *buf;
     int len;
{
  ...
  while (len > 0)
      ...
      if (inlen <= 0)
	{
	  ...
	  if (gdb_stdtargin == gdb_stdin && deprecated_readline_hook)
	    tmpbuf = (*deprecated_readline_hook) (gdbsim_prompt);
	  else if (gdb_stdtargin != gdb_stdin || input_from_terminal_p ())
	    {
	      if (SIGSETJMP (gdbsim_readline_env))
		{
		  signal (SIGINT, gdbsim_readline_prev_sigint);
		  quit_flag = 1;
		}
	      else
		{
		  gdbsim_readline_prev_sigint
		    = signal (SIGINT, input_hook_signal_handler);

		  if (gdb_stdtargin == gdb_stdin)
		    {
		      tmpbuf = gdb_readline_wrapper (gdbsim_prompt);
		    }
		  else
		    {
		      long ret;
...
/* GDB version of os_read callback.  */

static int 
gdb_os_read (p, fd, buf, len)
     host_callback *p;
     int fd;
     char *buf;
     int len;
{
  ...
  if (fd == 0)
    return gdb_os_read_stdin (p, buf, len);
  return default_callback.read (p, fd, buf, len);
}
...
_initialize_remote_sim (void)
{
  ...
  gdbsim_prompt = xstrdup ("");
  add_setshow_string_cmd ("simprompt", no_class, 
			   &gdbsim_prompt, _("\
Set simulator input prompt."), _("\
Show simulator input prompt."), _("\
Sets the prompt used when the program running on the GNU simulator\n\
performs a read from its console."),
			   NULL,
			   show_gdbsim_prompt,
			   &setlist, &showlist);
