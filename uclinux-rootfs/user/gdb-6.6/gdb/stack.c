	     ...
	     struct symtab_and_line sal)
{
  ...
  func = find_pc_sect_function (pc, section);
  if (func)
    {
...
return_command (char *retval_exp, int from_tty)
{
  ...
  struct value *return_value = NULL, *value = NULL;
  ...
  if (retval_exp)
	...
	value_fetch_lazy (return_value);
      ...
      value = allocate_value (value_type (return_value));
      if (thisfun != NULL)
	VALUE_ADDRESS (value) = BLOCK_START (SYMBOL_BLOCK_VALUE (thisfun));
...
If you continue, the return value that you specified will be ignored.\n";
      ...
      else if (using_struct_return (value, 0))
	{
	  query_prefix = "\
...
If you continue, the return value that you specified will be ignored.\n";
  ...
  if (return_value != NULL)
    {
      gdb_assert (gdbarch_return_value (current_gdbarch, value,
					NULL, NULL, NULL)
		  == RETURN_VALUE_REGISTER_CONVENTION);
      gdbarch_return_value (current_gdbarch, value,
