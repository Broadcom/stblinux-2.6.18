...
handle_inferior_event (struct execution_control_state *ecs)
{
  ...
  if (HAVE_CONTINUABLE_WATCHPOINT)
    ...
    stopped_by_watchpoint = (stepped_after_stopped_by_watchpoint == 1);
  ...
  if (stop_signal == TARGET_SIGNAL_TRAP
      ...
      if (stop_soon == STOP_QUIETLY_NO_SIGSTOP)
	{
      ...
      /* Don't even think about breakpoints if just proceeded over a
         breakpoint.  */
#if 0
      /* XXX Why, we might have landed on a new breakpoint? */
...
process_event_stop_test:
  ...
  /* Check for subroutine calls.  The check for the current frame
     ...
     NOTE: macro/2006-09-21: It also helps the case where previous
     frame's ID is the same as the current frame's ID, which may be
     the case when the frame has not been fully set up.
