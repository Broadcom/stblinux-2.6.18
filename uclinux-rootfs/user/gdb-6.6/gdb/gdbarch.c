...
gdbarch_return_value (struct gdbarch *gdbarch, struct value *function, struct regcache *regcache, gdb_byte *readbuf, const gdb_byte *writebuf)
{
  gdb_assert (gdbarch != NULL);
  ...
  return gdbarch->return_value (gdbarch, function, regcache, readbuf, writebuf);
}
