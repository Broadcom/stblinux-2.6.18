  ...
  for (p = ss; p != NULL; p = p->next)
    {
      ...
      if (static_only && p->symbol && p->block != STATIC_BLOCK)
	continue;
		      ...
		      int objc, Tcl_Obj *CONST objv[])
{
  ...
  */
  ...
  /* client_data.asm_argv[3] = cansetbreakpoint ? "-\t" : ""; */
  /* client_data.asm_argv[4] = cansetbreakpoint ? "break_rgn_tag" : ""; */
  /* client_data.asm_argv[5] = address; */
  /* client_data.asm_argv[6] = cansetbreakpoint ? "break_rgn_tag" : ""; */
  /* client_data.asm_argv[7] = offset; */
  /* client_data.asm_argv[8] = cansetbreakpoint ? "break_rgn_tag" : ""; */
		...
		struct disassemble_info *di)
{
  ...
  /*
   * Delete break_rgn_tag information if this instruction can not have
   * a breakpoint
   */
  if (!gdbarch_adjust_breakpoint_address_p (current_gdbarch) ||
      gdbarch_adjust_breakpoint_address (current_gdbarch, pc) == pc) {
    text_argv[3] = "-\t";
    text_argv[4] = "break_rgn_tag";
    text_argv[6] = "break_rgn_tag";
    text_argv[8] = "break_rgn_tag";
  }
  else {
    text_argv[3] = "\t";
    text_argv[4] = " ";
			...
			CORE_ADDR (*print_asm_fn) (ClientData, CORE_ADDR, struct disassemble_info *))
{
  ...
  if (mixed_source_and_assembly)
    {				/* Come here for mixed source/assembly */
      ...
      section = find_pc_overlay (low);
      if (pc_in_unmapped_range (low, section))
	{
	  low = overlay_mapped_address (low, section);
	  high = 1 + overlay_mapped_address (high - 1, section);
	  unmapped = 1;
	}

      /* Assume symtab is valid for whole PC range */
      sal = find_pc_sect_line (low, section, 0);
      symtab = sal.symtab;
	 ...
	 creating end_pc's and setting out_of_order as appropriate.  */
      ...
      if (i == nlines - 1
          && le[i].pc < high)
        {
          mle[newlines].line = le[i].line;
          mle[newlines].start_pc = le[i].pc;
          sal = find_pc_sect_line (le[i].pc, section, 0);
          ...
          for (pc = mle[i].start_pc; pc < mle[i].end_pc; )
            {
              QUIT;
	      if (unmapped)
		/* print using original unmapped address */
      ...
      if (unmapped)
	{
	  ...
	  high = 1 + overlay_unmapped_address (high - 1, section);
	}
      for (pc = low; pc < high; )
        {
          QUIT;
...
pc_function_name (CORE_ADDR pc)
{
  ...
  sym = find_pc_sect_function (pc, section);
  if (sym != NULL)
	       ...
	       int objc, Tcl_Obj *CONST objv[])
{
  ...
  /* This is not really correct.  Using core_addr_to_string_nz() will
     convert to hex and truncate to 32-bits when required but will
     otherwise not do what we really want. */
  Tcl_SetStringObj (result_ptr->obj_ptr, core_addr_to_string_nz (address), -1);
