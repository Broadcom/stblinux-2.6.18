	  ...
	  if (!junk && pc < BLOCK_END (block))
	    {
	      ...
	      xasprintf (&addr, "%s", core_addr_to_string_nz (BLOCK_START (block)));
	      Tcl_ListObjAppendElement (interp, elt,
					Tcl_NewStringObj (addr, -1));
	      free(addr);
	      xasprintf (&addr, "%s", core_addr_to_string_nz (BLOCK_END (block)));
	      Tcl_ListObjAppendElement (interp, elt,
					Tcl_NewStringObj (addr, -1));
		    ...
		    int objc, Tcl_Obj *CONST objv[])
{
  ...
  if (deprecated_selected_frame == NULL)
  ...
  else
    {
      struct block *block;
      block = get_frame_block (deprecated_selected_frame, 0);
      xasprintf (&start, "%s", core_addr_to_string_nz (BLOCK_START (block)));
      xasprintf (&end, "%s", core_addr_to_string_nz (BLOCK_END (block)));
    }
		    ...
		    int objc, Tcl_Obj *CONST objv[])
{
  ...
  if (deprecated_selected_frame == NULL)
    ...
    xasprintf (&frame, "%s",
	       core_addr_to_string_nz (get_frame_base (deprecated_selected_frame)));
