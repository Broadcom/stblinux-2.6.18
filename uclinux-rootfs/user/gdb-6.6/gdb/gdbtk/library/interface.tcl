...
proc gdb_run_readline_command_no_output {command args} {
  ...
  gdb_cmd $command
  # reset readlineShowUser to default
  set gdbtk_state(readlineShowUser) 1
}
...
#  debug
  ...
  if {![info exists gdbtk_state(readlineArgs)] && $gdbtk_state(readlineShowUser)} {
    if {$gdbtk_state(console) == ""} {
      debug "creating new console window"
      ManagedWin::open Console
    }
    #debug "popping console window"
    #$gdbtk_state(console) reveal
    $gdbtk_state(console) insert $message    
  }
}
...
#  debug
  if {![info exists gdbtk_state(readlineArgs)]} {
    if {$gdbtk_state(console) != ""} {
      debug "readline end"
      debug "calling console activate null"
      $gdbtk_state(console) activate
    }
  }
...
#  debug "target=$target pretty_name=$gdb_pretty_name"
  set targ_opts ""
  # For the simulator the options are appended to the target command
  if {$gdb_target_name == "sim"} {
      catch [set targ_opts [pref getd gdb/load/${gdb_target_name}-opts]]
      debug "targ_opts=$targ_opts gdb_target_name=$gdb_target_name targ=$targ"
  }
  ...
  set mdidev ""
  catch [set mdidev [pref getd gdb/load/$target-mdidev]]
  if {![catch {pref getd gdb/load/$target-mdicfg} cfg]} {
    if {$cfg != ""} {
      set mdidev "$mdidev,cfg=$cfg"
    }
  }
  if {![catch {pref getd gdb/load/$target-mdirstconn} rst]} {
    if {$rst == ""} {
      set rst "off"
    } elseif {$rst == 0} {
      set rst "off"
      ...
      set rst ""
      catch {pref getd gdb/load/$target-mdirstsecs} rst
      if {$rst == ""} {
	set rst "on"
      }
    }
    set mdidev "$mdidev,rst=$rst"
  }
  ...
  # replace "mdiX" with mdidev,options,...
  set targ [lrep $targ "mdiX" $mdidev]
...
proc set_target {} {
  ...
  if {$gdb_target_cmd == "" && ![TargetSelection::native_debugging]} {
    if {$gdb_target_name == ""} {
      ...
      if {[TargetSelection::getname $gdb_target_name pretty-name] == ""} {
	set gdb_target_name ""
      }
    } else {
      set prompt 0
...
proc gdbtk_stop {} {
  ...
  if {$_gdbtk_stop(timer) == ""} {
    ...
    if {$gdbtk_state(readline) != 0} {
      # Terminate any pending readline
      set gdbtk_state(readline_response) ""
    }
    catch {gdb_stop}
  }
}
