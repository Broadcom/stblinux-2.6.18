...
mdi_open (char *args, int from_tty)
{
  ...
  if (TARGET_BYTE_ORDER != byte_order)
	...
	? "big" : "little";
      ...
      if (TARGET_BYTE_ORDER == BFD_ENDIAN_UNKNOWN && ! exec_bfd)
	{
...
Do you want to start executing at 0x%s anyway? ", paddr_nz (entry_point)))
  ...
  write_register (MIPS_A0_REGNUM + 0, 0);	/* $a0 */
  write_register (MIPS_A0_REGNUM + 1, 0);	/* $a1 */
  write_register (MIPS_A0_REGNUM + 2, 0);	/* $a2 */
  write_register (MIPS_A0_REGNUM + 3, 0);	/* $a3 */
  write_register (MIPS_SP_REGNUM, 0);		/* $sp */

  /* Ensure interrupts are disabled. */
  write_register (MIPS_FIRST_EMBED_REGNUM + 12, 
		  read_register (MIPS_FIRST_EMBED_REGNUM + 12) & ~1);
  ...
  while (len > 0)
      ...
      if (inlen <= 0)
	{
	  ...
	  if (gdb_stdtargin == gdb_stdin && deprecated_readline_hook)
		...
		{
		  mdi_readline_prev_sigint
		    = signal (SIGINT, mdi_readline_signal_handler);
...
mdi_memresource (CORE_ADDR addr)
{
  ...
  switch (mdi_asidtype)
    ...
    case ASIDReg:
      /* FIXME: EntryHI */
      rsrc = MDIMIPVIRTUAL
	+ (read_register (MIPS_FIRST_EMBED_REGNUM + 10) & 0xff);
      break;
    case ASIDFixed:
...
mdi_stopped_data_address (struct target_ops *target, CORE_ADDR *addr_p)
{
  ...
  if (mdi_stopped_by_watchpoint() || stepped_after_stopped_by_watchpoint)
    {
  ...
  switch (mdi_desc->tlb)
	...
	if (entry < 0)
	...
	else
	  {
	    ...
	    if (mdistat != MDISuccess)
	      return mdistat;
	    lo = extract_unsigned_integer ((gdb_byte *)&data[0], 4);
	    hi = extract_unsigned_integer ((gdb_byte *)&data[1], 4);
	    if (remote_debug)
	      fprintf_unfiltered (mdi_logfp,
	...
	if (entry < 0)
	...
	else
	  {
	    ...
	    if (mdistat != MDISuccess)
	      return mdistat;
	    lo0 = extract_unsigned_integer ((gdb_byte *)&data[0], 4);
	    lo1 = extract_unsigned_integer ((gdb_byte *)&data[1], 4);
	    hi = extract_unsigned_integer ((gdb_byte *)&data[2], 4);
	    mask = extract_unsigned_integer ((gdb_byte *)&data[3], 4);
	    if (remote_debug)
	      fprintf_unfiltered 
		(mdi_logfp,
	...
	if (entry < 0)
	...
	else
	  {
	    ...
	    if (mdistat != MDISuccess)
	      return mdistat;
	    lo0 = extract_unsigned_integer ((gdb_byte *)&data[0], 8);
	    lo1 = extract_unsigned_integer ((gdb_byte *)&data[1], 8);
	    hi = extract_unsigned_integer ((gdb_byte *)&data[2], 8);
	    mask = extract_unsigned_integer ((gdb_byte *)&data[3], 8);
	    if (remote_debug)
	      fprintf_unfiltered
		(mdi_logfp,
		 "%2d: hi=0x%08lx lo0=0x%08lx lo1=0x%08lx mask=0x%08lx\n", 
...
set_tlb_command (char *args, int from_tty)
{
  ...
  switch (mdi_desc->tlb)
	...
	entry = data[0];
	store_unsigned_integer ((gdb_byte *)&tlb[0], sizeof (tlb[0]), data[1]);
	store_unsigned_integer ((gdb_byte *)&tlb[1], sizeof (tlb[1]), data[2]);
	mdistat = mdiWrite (mdi_desc->gmdata[mdi_desc->gmindex].DevHandle,
			    MDIMIPTLB, entry,
			    tlb, sizeof (tlb[0]), 2);
	...
	entry = data[0];
	store_unsigned_integer ((gdb_byte *)&tlb[0], sizeof (tlb[0]), data[2]);
	store_unsigned_integer ((gdb_byte *)&tlb[1], sizeof (tlb[1]), data[3]);
	store_unsigned_integer ((gdb_byte *)&tlb[2], sizeof (tlb[2]), data[1]);
	store_unsigned_integer ((gdb_byte *)&tlb[3], sizeof (tlb[3]), data[4]);
	mdistat = mdiWrite (mdi_desc->gmdata[mdi_desc->gmindex].DevHandle,
  ...
  while (nsets-- != 0)
    {
      ...
      tag = extract_unsigned_integer ((gdb_byte *)&info[0], 4);
      parity = extract_unsigned_integer ((gdb_byte *)&info[1], 4);
      ...
      for (j = 0; j < (cache->LineSize / 4); j++)
	{
	  ULONGEST v;
	  char cdata[8];
	  v = extract_unsigned_integer ((gdb_byte *)&info[j], 4);
	  if ((j & 3) == 0)
	    printf_filtered ("\n");
...
show_cp0_command (char *args, int from_tty)
{
  ...
  switch (regno)
    ...
    case 8:			/* BadVaddr */
      ...
      break;
    case 12:
      regno = MIPS_PS_REGNUM;
      break;
    case 13:
      regno = regnum->cause;
      break;
    default:
      regno += MIPS_FIRST_EMBED_REGNUM;
      break;
    }
...
set_cache1 (int which, char *args)
{
  ...
  for (i = 0; i < cache->LineSize / 4; i++)
	...
	args++;
      ...
      store_unsigned_integer ((gdb_byte *)&info[i], 4, data);
    }
  ...
  store_unsigned_integer ((gdb_byte *)&tagbuf[0], 4, (ULONGEST) tag);
  store_unsigned_integer ((gdb_byte *)&tagbuf[1], 4, (ULONGEST) parity);
  mdistat = mdiWrite (mdi_desc->gmdata[mdi_desc->gmindex].DevHandle,
...
function."),
  ...
  deprecated_add_show_cmd ("implementer", no_class, var_string,
			   (void *)&mdi_implementer, _("\
...
which gdb will use to qualify a target virtual address."),
  ...
  add_cmd ("tlb", class_support, show_tlb_command, _("\
Show TLB entries.\n\
...
the entire TLB is diplayed."), 
  ...
  add_cmd ("tlb", class_support, set_tlb_command, _("\
Set TLB entry.\n\
...
displayed after it has been written."), 
  ...
  add_cmd ("cp0", class_obscure, show_cp0_command, _("\
Show CP0 register.\n\
...
CP0 register."), 
  ...
  add_cmd ("cp0", class_support, set_cp0_command, _("\
Set CP0 register.\n\
...
gdb does not know about a particular CP0 register."),
  ...
  add_cmd ("dcache", class_support, show_dcache_command, _("\
...
entries for the line are displayed, followed by the cache line data."),
  ...
  add_cmd ("dcache", class_support, set_dcache_command, _("\
...
command."), 
  ...
  add_cmd ("icache", class_support, show_icache_command, _("\
...
entries for the line are displayed, followed by the cache line data."),
  ...
  add_cmd ("icache", class_support, set_icache_command, _("\
...
command."), 
  ...
  add_cmd ("scache", class_support, show_scache_command, _("\
...
entries for the line are displayed, followed by the cache line data."),
  ...
  add_cmd ("scache", class_support, set_scache_command, _("\
...
command."), 
  ...
  add_cmd ("devices", class_support, mdi_show_devices_command,
	   _("List all available MDI devices."), 
	   &showmdicmdlist);
