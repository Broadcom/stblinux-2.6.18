...
call_function_by_hand (struct value *function, int nargs, struct value **args)
{
  ...
  value = allocate_value (values_type);
  VALUE_ADDRESS (value) = funaddr;
  struct_return = using_struct_return (value, using_gcc);
...
the function call)."), name);
  ...
  /* Figure out the value returned by the function.  */
  {
	...
	switch (gdbarch_return_value (arch, value, NULL, NULL, NULL))
	  {
	  case RETURN_VALUE_REGISTER_CONVENTION:
	  case RETURN_VALUE_ABI_RETURNS_ADDRESS:
	  case RETURN_VALUE_ABI_PRESERVES_ADDRESS:
	    retval = allocate_value (values_type);
	    gdbarch_return_value (current_gdbarch, value, retbuf,
