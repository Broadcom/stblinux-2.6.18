...
static void print_return_value (int struct_return, struct value *function);
...
static void
print_return_value (int struct_return, struct value *function)
{
  ...
  CHECK_TYPEDEF (type);
  gdb_assert (TYPE_CODE (type) != TYPE_CODE_VOID);
  ...
  switch (gdbarch_return_value (gdbarch, function, NULL, NULL, NULL))
    {
    case RETURN_VALUE_REGISTER_CONVENTION:
    case RETURN_VALUE_ABI_RETURNS_ADDRESS:
    case RETURN_VALUE_ABI_PRESERVES_ADDRESS:
      value = allocate_value (type);
      gdbarch_return_value (current_gdbarch, function, stop_registers,
  ...
  if (value)
  ...
  else
    {
      ...
      ui_out_field_string (uiout, "return-type", TYPE_NAME (type));
      ui_out_text (uiout, ".");
...
finish_command_continuation (struct continuation_arg *arg)
{
  ...
  if (bpstat_find_breakpoint (stop_bpstat, breakpoint) != NULL
      ...
      if (TYPE_CODE (value_type) == TYPE_CODE_VOID)
	{
      ...
      VALUE_ADDRESS (value) = BLOCK_START (SYMBOL_BLOCK_VALUE (function));
      struct_return = using_struct_return (value, gcc_compiled);

      print_return_value (struct_return, value); 
    }
	...
	if (regnum >= 0)
	  {
	    /* User registers lie completely outside of the range of normal
	       ...
	       Apply the same change below when register is a number?  */
	    if (regnum >= NUM_REGS + NUM_PSEUDO_REGS)
	      {
		struct value *val
		  = value_of_user_reg (regnum, get_selected_frame (NULL));
