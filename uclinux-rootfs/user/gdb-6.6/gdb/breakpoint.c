		    ...
		    int *hw_breakpoint_error)
{
  ...
  if (bpt->loc_type == bp_loc_software_breakpoint
      || bpt->loc_type == bp_loc_hardware_breakpoint)
    {
	  ...
	  || bpt->section == NULL
	  || !(section_is_real_overlay (bpt->section)))
	{
  ...
  if (b->loc_type == bp_loc_software_breakpoint
      || b->loc_type == bp_loc_hardware_breakpoint)
    {
	 ...
	 bp_hardware_breakpoint.  */
      ...
      if (overlay_debugging == ovly_off
	  || b->section == NULL
	  || !(section_is_real_overlay (b->section)))
	{
...
bpstat_stop_status (CORE_ADDR bp_addr, ptid_t ptid, int stopped_by_watchpoint)
{
  ...
  ALL_BREAKPOINTS_SAFE (b, temp)
  {
    ...
    if (b->type == bp_hardware_watchpoint
	 ...
	 || b->type == bp_access_watchpoint)
      {
	/* But if we hit the associated scope breakpoint we process the
	   watchpoint anyway.  */
	if (!stopped_by_watchpoint
	    && (!b->related_breakpoint
		|| b->related_breakpoint->type != bp_watchpoint_scope
		|| b->related_breakpoint->loc->address != bp_addr))
	  continue;
      }
...
set_raw_breakpoint (struct symtab_and_line sal, enum bptype bptype)
{
  ...
  /*
   * b->type must be valid before adjust_breakpoint_address which may end up reading
   * memory using deprecated_read_memory_nobpt
   */
  b->type = bptype;
      ...
      ALL_BREAKPOINTS_SAFE (b, tmp)
	{
	  ...
	  if (b->type != bp_none
	      ...
	      && (((sal.pc && (b->loc->address == sal.pc)) 
		   && (!section_is_real_overlay (b->loc->section)
		       || b->loc->section == sal.section))
